...
if(!empty($mk))
      {
		...
		$dt_mhs = new dt_mhs;
		  ...
		  $dt_stat_mhs->insert_perwalian($noktp,1,$mhsudate,'system',$fak,$kd_prodi,$ta);
		  ...
		  {			   
			$sem = $dt_mtk->carisem($ta,$fak,$kd_prodi,$kdkmk);             			
           	$dt_krs->save_krs(array('thsmskrs'=>$ta,'fak'=>$fak,'prodi'=>$kd_prodi,'semkrs'=>$sem,'shiftkrs'=>$shift,'kelaskrs'=>$kls,'nokmkkrs'=>$kdkmk,'noktphskrs'=>$noktp,'kampus'=>$kmps,'mhsudate'=>$mhsudate));
            if($dt_mtk->kd_lab!="-"){			  
			  $dt_krs->save_krs(array('thsmskrs'=>$ta,'fak'=>$fak,'prodi'=>$kd_prodi,'semkrs'=>$sem,'shiftkrs'=>$shift,'kelaskrs'=>$kls,'nokmkkrs'=>$dt_mtk->mtk_no,'noktphskrs'=>$noktp,'kampus'=>$kmps,'mhsudate'=>$mhsudate));
            }			  
